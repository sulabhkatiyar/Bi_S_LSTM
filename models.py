...
class DecoderWithAttention_choice(nn.Module):
    def __init__(self, embed_dim, decoder_dim, vocab_size, encoder_dim=2048, dropout=0.5):
        ...
        self.decode_step1a = nn.LSTMCell(decoder_dim, decoder_dim, bias=True)
        ...
        self.decode_step_reverse1a = nn.LSTMCell(decoder_dim, decoder_dim, bias=True)
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths):
        ...
        ha, ca = torch.zeros_like(img_f), torch.zeros_like(img_f)
        h1, c1 = torch.zeros_like(img_f), torch.zeros_like(img_f)
        hr, cr = torch.zeros_like(img_f), torch.zeros_like(img_f)
        hra, cra = torch.zeros_like(img_f), torch.zeros_like(img_f)
        ...
        for t in range(max(decode_lengths)):
            ...
            ha, ca = self.decode_step1a(h[:batch_size_t], (ha[:batch_size_t], ca[:batch_size_t]))     
            h1, c1 = self.decode_step2(torch.cat([ha[:batch_size_t], img_f[:batch_size_t]], dim = 1),(h1[:batch_size_t], c1[:batch_size_t]))     
        ...
        for t in range(max(decode_lengths)):
            ...
            hra, cra = self.decode_step_reverse1a(hr[:batch_size_t], (hra[:batch_size_t], cra[:batch_size_t]))     
            hr1, cr1 = self.decode_step_reverse2(torch.cat([hra[:batch_size_t], img_r[:batch_size_t]], dim = 1),(hr1[:batch_size_t], cr1[:batch_size_t]))     
